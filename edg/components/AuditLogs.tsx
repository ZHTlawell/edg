...
export const AuditLogs: React.FC = () => {
    ...
    const handleExportLogs = (logsToExport: typeof logs) => {
        ...
        const headers = ['时间戳', '操作员', '角色', '事件概要', '终端IP', '状态'];
        const rows = filtered.map(l => [l.time, l.user, l.role, l.action, l.ip, l.status]);
        const csv = [headers, ...rows].map(r => r.map(v => `"${v}"`).join(',')).join('\n');
        const blob = new Blob(['\uFEFF' + csv], { type: 'text/csv;charset=utf-8;' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url; a.download = `审计日志_${new Date().toISOString().split('T')[0]}.csv`;
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <button onClick={() => handleExportLogs(logs)} className="flex items-center gap-2 bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 px-5 py-2.5 rounded-xl font-bold transition-all shadow-sm self-start md:self-auto">
