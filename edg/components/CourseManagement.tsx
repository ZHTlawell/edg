...
export const CourseManagement: React.FC = () => {
  ...
  const handleExportCourses = () => {
    if (!courses || courses.length === 0) {
      addToast('暂无课程数据可导出', 'warning');
      return;
    }
    const headers = ['课程ID', '课程编号', '课程名称', '分类', '等级', '价格', '总课时', '状态', '校区', '主讲教师'];
    const rows = courses.map((c: any) => [
      c.id, c.code || '', c.name, c.category, c.level || '', c.price, c.totalLessons, c.status, c.campus || '', c.instructor || ''
    ]);
    ...
    link.setAttribute('download', `课程数据_${new Date().toISOString().split('T')[0]}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    addToast(`已导出 ${courses.length} 条课程数据`, 'success');
  };
  ...
  const handleToggleStatus = async (id: string) => {
    ...
    setCourses((courses || []).map(c =>
      c.id === id ? { ...c, status: newStatus as CourseStatus } : c
    ));
    try {
      await api.patch('/api/academic/courses/' + id, { status: newStatus });
    } catch (e: any) {
      // Revert on failure
      setCourses((courses || []).map(c =>
        c.id === id ? { ...c, status: course.status } : c
      ));
      addToast(e.message || '状态更新失败', 'error');
    }
  };
