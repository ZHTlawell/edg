...
export const StudentManagement: React.FC<StudentManagementProps> = ({ onShowDetail }) => {
  ...
  const handleExportStudents = () => {
    ...
    const headers = ['学员ID', '姓名', '性别', '手机号', '校区', '状态', '注册日期'];
  ...
  const handleDownloadTemplate = () => {
    ...
    const link = document.createElement('a');
    link.setAttribute('href', url);
    link.setAttribute('download', '学员导入模板.csv');
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    ...
    <div className="space-y-6 animate-fade-in relative">
      ...
      {deletingStudent && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-slate-900/60" onClick={() => setDeletingStudent(null)}></div>
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 bg-white border border-slate-200 text-slate-600 px-6 py-3.5 rounded-2xl text-sm font-bold hover:bg-slate-50 transition-all shadow-sm active:scale-95"
          >
