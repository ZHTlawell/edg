        ...
        <div className="fixed inset-0 z-[100] flex justify-end">
          ...
          <div className="relative w-full max-w-lg bg-white h-screen shadow-2xl animate-in slide-in-from-right duration-500 overflow-y-auto custom-scrollbar">
            ...
            <div className="p-8 space-y-10">
              ...
              <div className="bg-slate-900 rounded-[2rem] p-8 text-white space-y-6 shadow-xl shadow-slate-200">
                ...
                {!isStudent && (
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <button onClick={() => setIsRescheduleModalOpen(true)} className="flex items-center justify-center gap-2 py-3.5 bg-white/10 hover:bg-white/20 text-white rounded-2xl text-xs font-bold transition-all border border-white/10 active:scale-95">
                      <ElmIcon name="clock" size={16} /> 调整时间
                    </button>
                    <button onClick={() => setIsCancelConfirmOpen(true)} className="flex items-center justify-center gap-2 py-3.5 bg-red-500/10 hover:bg-red-500/20 text-red-400 rounded-2xl text-xs font-bold transition-all border border-red-500/20 active:scale-95">
                      <ElmIcon name="close" size={16} /> 取消课次
                    </button>
                  </div>
                )}
                {isStudent && (
                  <button onClick={() => { setLeaveReason(''); setIsLeaveModalOpen(true); }} className="w-full py-4 bg-emerald-500 text-white rounded-2xl text-sm font-bold shadow-lg shadow-emerald-100 hover:bg-emerald-600 transition-all flex items-center justify-center gap-2">
              ...
              {/* Student List Entry */}
              <button onClick={() => setIsAttendanceListOpen(true)} className="w-full p-6 bg-slate-50 hover:bg-white border border-slate-100 hover:border-blue-100 rounded-3xl transition-all group flex items-center justify-between hover:shadow-lg">
          ...
          <div className="relative bg-white rounded-[2.5rem] shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-300">
            ...
            <div className="px-8 py-6 border-t border-slate-100 flex justify-end gap-3">
              ...
              <button
                ...
                className="px-8 py-3 bg-emerald-500 text-white rounded-2xl text-xs font-bold hover:bg-emerald-600 transition-all"
              >提交申请</button>
            </div>
          </div>
        </div>
