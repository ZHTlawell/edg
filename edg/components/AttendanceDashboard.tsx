...
export const AttendanceDashboard: React.FC<AttendanceDashboardProps> = ({ onRegister, onNavigate }) => {
  ...
  const absenceAlerts = useMemo(() => {
    if (attendanceRecords.length === 0) return [];
    // Group records by student, sorted by date asc
    const byStudent: Record<string, any[]> = {};
    attendanceRecords.forEach(r => {
      if (!byStudent[r.student_id]) byStudent[r.student_id] = [];
      byStudent[r.student_id].push(r);
    });
    const alerts: { name: string; className: string; status: string; studentId: string }[] = [];
    Object.entries(byStudent).forEach(([sid, recs]) => {
      const sorted = [...recs].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
      // Count trailing consecutive absences
      let consecutive = 0;
      ...
      if (consecutive >= 1) {
        ...
        const cls = (classes || []).find((c: any) => c.id === sorted[0]?.class_id);
        const name = student?.name || `学员 ${sid.slice(-4)}`;
        const className = cls?.name || '未知班级';
        const statusLabel = consecutive >= 2 ? `连续缺勤${consecutive}次` : '今日未到课';
  ...
  const handleExportAttendance = () => {
    if (attendanceRecords.length === 0) {
      addToast('暂无考勤数据可导出', 'warning');
      return;
    }
    const headers = ['课次ID', '学员ID', '考勤状态', '扣课时', '日期'];
    const rows = attendanceRecords.map((r: any) => [
      r.lesson_id, r.student_id, r.status,
      r.deductHours ?? 0,
    ...
    link.setAttribute('download', `考勤分析报表_${new Date().toISOString().split('T')[0]}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    addToast('考勤数据已导出', 'success');
  };
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button onClick={handleExportAttendance} className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 text-slate-600 rounded-xl text-sm font-bold hover:bg-slate-50 transition-all shadow-sm">
            <ElmIcon name="download" size={16} /> 导出分析
          </button>
          <button
            onClick={() => onNavigate?.('teaching')}
            className="flex items-center gap-2 px-5 py-2 bg-white border border-slate-200 text-slate-600 rounded-xl text-sm font-bold hover:bg-slate-50 transition-all shadow-sm active:scale-95"
          >
            <ElmIcon name="calendar" size={16} /> 前往课表管理
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm space-y-4">
          ...
          <div>
            ...
            <h3 className="text-2xl font-bold text-slate-900 font-mono tracking-tighter mt-1">{stats.rate}%</h3>
          </div>
        </div>
        <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm space-y-4">
          <div className="flex items-center justify-between">
          ...
          <div>
            ...
            <h3 className="text-2xl font-bold text-slate-900 font-mono tracking-tighter mt-1">{stats.leave}</h3>
          </div>
        </div>
        <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm space-y-4">
          <div className="flex items-center justify-between">
          ...
          <div className="bg-white rounded-[2rem] border border-slate-100 p-8 shadow-sm space-y-6">
            ...
            <div className="space-y-4">
              {absenceAlerts.length === 0 ? (
                <div className="py-6 text-center text-slate-400 text-xs font-medium">暂无缺勤异常学员 🎉</div>
              ) : absenceAlerts.map((alert, i) => (
                <div key={i} className="flex gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-100 hover:border-red-100 transition-colors cursor-pointer">
                  <div className="w-10 h-10 rounded-xl bg-white flex items-center justify-center font-bold text-slate-500 shadow-sm text-sm">
                    {alert.name.charAt(0)}
                  </div>
                  <div className="flex-1 space-y-0.5">
                    <div className="flex justify-between items-center">
                      <p className="text-sm font-bold text-slate-800">{alert.name}</p>
                      <span className="text-[9px] font-bold text-red-500">{alert.status}</span>
                    </div>
                    <p className="text-[10px] text-slate-400 font-medium">{alert.className}</p>
                  </div>
                </div>
              ))}
            </div>
