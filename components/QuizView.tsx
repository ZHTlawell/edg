        ...
        <div className="flex items-center gap-4">
           <button 
             ...
             className={`flex items-center gap-2 px-5 py-2.5 rounded-2xl text-xs font-bold transition-all border ${
               isFlagged[currentQuestion.id] ? 'bg-amber-50 border-amber-500 text-amber-600' : 'bg-white border-slate-200 text-slate-400 hover:border-slate-400'
             }`}
           >
           ...
           <div className="space-y-6">
              <div className="flex items-center gap-3">
                 <span className={`px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider ${
                   currentQuestion.type === 'single' ? 'bg-blue-50 text-blue-600 border border-blue-100' : 'bg-purple-50 text-purple-600 border border-purple-100'
                 }`}>
                 ...
                 {currentQuestion.options.map((opt) => {
                    ...
                    return (
                      <button
                        ...
                        className={`group w-full flex items-center gap-6 p-6 rounded-[2rem] border-2 transition-all text-left relative overflow-hidden ${
                          isSelected 
                      ...
                      >
                         <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-bold text-lg shrink-0 transition-colors ${
                           isSelected ? 'bg-white/20 text-white' : 'bg-slate-50 text-slate-400 group-hover:bg-blue-50 group-hover:text-blue-600'
                         }`}>
                           {opt.label}
                         </div>
                 ...
                 {MOCK_QUESTIONS.map((q, idx) => (
                   <button
                     ...
                     className={`w-10 h-10 rounded-xl flex items-center justify-center text-xs font-bold transition-all relative ${
                       currentIdx === idx ? 'ring-4 ring-blue-50 border-2 border-blue-600 text-blue-600 scale-110 z-10 bg-white' :
         ...
         <div className="flex items-center gap-4">
            ...
            {isLastQuestion ? (
              <button 
                ...
                className="px-12 py-3.5 bg-blue-600 text-white rounded-2xl text-sm font-bold shadow-xl shadow-blue-100 hover:bg-blue-700 transition-all active:scale-95 flex items-center gap-2"
              >
                <Send size={18} /> 提交测验
              </button>
            ) : (
