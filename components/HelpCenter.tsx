...
export const HelpCenter: React.FC = () => {
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500 pb-10">
            {/* Hero Section */}
            <div className="bg-gradient-to-br from-blue-600 to-indigo-800 rounded-3xl p-8 md:p-12 text-center text-white shadow-2xl shadow-blue-900/20 relative overflow-hidden">
                {/* Decorative elements */}
                <div className="absolute -top-24 -right-24 w-64 h-64 bg-white/10 rounded-full blur-3xl mix-blend-overlay"></div>
                <div className="absolute -bottom-24 -left-24 w-64 h-64 bg-blue-400/20 rounded-full blur-3xl mix-blend-overlay"></div>

                <div className="relative z-10 max-w-2xl mx-auto space-y-6">
                    <div className="mx-auto w-16 h-16 bg-white/10 backdrop-blur-md rounded-2xl flex items-center justify-center border border-white/20 shadow-xl mb-6">
                        <HelpCircle size={32} className="text-blue-100" />
                    </div>
                    <h1 className="text-3xl md:text-4xl font-bold tracking-tight">你好！这里是 EduAdmin 帮助中心</h1>
                    <p className="text-blue-100 text-lg">快速查找产品手册、教学视频，或向您的专属顾问提问求助。</p>

                    <div className="relative mt-8">
                        <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={20} />
                        <input
                            type="text"
                            placeholder="请描述您遇到的问题... 例如：如何排课"
                            className="w-full bg-white rounded-2xl py-4 pl-12 pr-6 outline-none focus:ring-4 focus:ring-blue-400/30 transition-shadow text-slate-800 font-medium text-lg placeholder:text-slate-400 shadow-xl"
                            value={searchQuery}
                            onChange={(e) => setSearchQuery(e.target.value)}
                        />
                        <button className="absolute right-2 top-1/2 -translate-y-1/2 bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-xl font-bold transition-colors">
                            搜 索
                        </button>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[
                    { icon: <BookOpen size={24} />, title: '新手入驻指南', desc: '5分钟快速梳理基础机构与系统数据结构设定', color: 'blue' },
                    { icon: <PlayCircle size={24} />, title: '视频点播教程', desc: '手把手操作录屏，掌握极速排课与订单收取的姿势', color: 'indigo' },
                    { icon: <MessageCircle size={24} />, title: '联系在线客服', desc: '工作日 9:00 - 18:00 提供真人企业微信服务支持', color: 'emerald' },
                ].map((item, i) => (
